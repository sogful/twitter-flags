(function () {
  ...
  let cfg;
  try { cfg = Object.assign({}, DEFAULT, JSON.parse(localStorage.getItem(KEY) || "{}")) } catch { cfg = Object.assign({}, DEFAULT) }
  ...
  function cssensure() {
    ...
    .tfdev-tip {position: fixed; z-index: 2147483647; pointer-events: none;
      background: #1d9bf0; color: #fff; font: 12px TwitterChirp, system-ui, sans-serif;
      padding: 3px 8px; border-radius: 6px; transform: translateY(-130%);
  ...
  function onkeypress(e) { if (e.key === "Escape") { cfg.inspect = false; save(); apply(); post(); log("inspector off (esc)") } }
  function onclick(e) {
  ...
  function inspectset(on) {
    ...
    if (on) {
      if (!tip && document.body) { tip = document.createElement("div"); tip.className = "tfdev-tip"; document.body.appendChild(tip) }
  ...
  function apply() {
    cssensure();
    try {if (cfg.jfDev) sessionStorage.setItem("jfDev", "true"); else if (sessionStorage.getItem("jfDev") === "true") sessionStorage.removeItem("jfDev")} catch {}
    exposedebug(!!cfg.exposeDebug);
